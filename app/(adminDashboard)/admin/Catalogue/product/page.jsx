...
import { Download, DownloadIcon, FileSearch, Plus, Search, SearchIcon, Upload } from 'lucide-react';
    ...
    <SearchIcon className='text-gray-500' size={19}/>
      ...
      <div className="mb-4 grid grid-cols-1 lg:grid-cols-2 gap-x-10">
        ...
        {Object.keys(tempSearchFields).map(field => (
          <div key={field} className="flex items-center mb-2">
            ...
            {field === 'Category' || field === 'Product type' || field === 'Subcategory' ? (
      <select
      ...
      <table className="min-w-full text-left">
        ...
        <tr>
            ...
            Picture {sortConfig.key === 'username' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
            </th>
            <th className="  px-4 py-2 cursor-pointer" onClick={() => handleSort('email')}>
            PRODUCTNAME {sortConfig.key === 'email' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
            </th>
            <th className=" px-4 py-2 cursor-pointer" onClick={() => handleSort('phone')}>
             PRICE {sortConfig.key === 'phone' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
            </th>
            <th className=" px-4 py-2 cursor-pointer" onClick={() => handleSort('phone')}>
            STOCK QUANTITY {sortConfig.key === 'phone' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
            </th>
            <th className=" px-4 py-2 cursor-pointer" onClick={() => handleSort('phone')}>
            PUBLISHED {sortConfig.key === 'phone' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
            </th>
            <th className=" px-4 py-2 cursor-pointer" onClick={() => handleSort('phone')}>
