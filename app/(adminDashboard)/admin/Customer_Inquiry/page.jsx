      ...
      <div className="mb-4 grid grid-cols-1 lg:grid-cols-2 gap-x-10">
        ...
        {Object.keys(tempSearchFields).map(field => (
          <div key={field} className="flex items-center mb-2">
            <label className="w-1/6 font-medium mr-2">{field}</label>
      ...
      <table className="min-w-full text-left">
        ...
        <thead>
          <tr>
            ...
            From {sortConfig.key === 'userId' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
            </th>
            <th className="px-2 py-2 cursor-pointer" onClick={() => handleSort('id')}>
            Subject {sortConfig.key === 'id' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
            </th>
            <th className="px-2 py-2 cursor-pointer" onClick={() => handleSort('title')}>
            Received on  {sortConfig.key === 'title' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''} 
        ...
        <tbody>
          {slicedData.map((item) => (
            <tr key={item.id}>
              ...
              <Button color="warning" className="shadow-md" ><Eye size={20} className='pr-1' />View</Button>
              </td>
            </tr>
          ))}
        </tbody>
...
export default customerinquirt;